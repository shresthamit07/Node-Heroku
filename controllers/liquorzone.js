...
router.get('/contact_us', function(req, res, next) {
  if(req.user != undefined && req.user.is_admin){
    ...
    if(req.isAuthenticated()){
	   res.render('t_contact', {session: req.isAuthenticated(), id: req.user.id, name: req.user.first_name, url: req.url});
   }else{
    res.render('t_contact', {session: req.isAuthenticated(), url: req.url});
   }
  }
})
...
router.get('/about_us', function(req, res, next) {
  if(req.user != undefined && req.user.is_admin){
    ...
    if(req.isAuthenticated()){
     res.render('t_about_us', {session: req.isAuthenticated(), id: req.user.id, name: req.user.first_name, url: req.url});
   }else{
    res.render('t_about_us', {session: req.isAuthenticated(), url: req.url});
   }
  }
})
  ...
  if(req.user != undefined && req.user.is_admin){
	...
	if(country_param != undefined){
      ...
      client.query('SELECT * from ratings', function (err, rate_result) {
        ...
        ratings_in_hash_format = get_rating_hash(ratings_value)
    	if(req.isAuthenticated()){
	      	res.render('t_search_result.ejs',{data: items, ratings: ratings_in_hash_format, search_param: search_param, session: req.isAuthenticated(), id: req.user.id, name: req.user.first_name, url: req.url});
	      }
	      else{
          res.render('t_products.ejs',{data: items, ratings: ratings_in_hash_format, search_param: search_param, session: req.isAuthenticated(), url: req.url});
	      }
      })
    	})	
